...
A standalone GCP pipeline that ingests CTD Module 3 PDFs, extracts structured pharmaceutical data using Claude (Vertex AI), and outputs Neo4j-ready graph artefacts.

**Repository:** `git@github.com:sescapa-normacy/pdf-pipeline.git`
...
```bash
git clone git@github.com:sescapa-normacy/pdf-pipeline.git
cd pdf-pipeline
```
...
Grant Secret Manager access to the function service accounts (Terraform handles this for the final deploy, but the secret must exist before running `terraform apply`).
...
Copy the generated key string — it is used as `x-api-key` in all API requests and in the frontend configuration.

### 6. Configure the frontend

Set these environment variables in the Normacy frontend:
...
After any code changes to a Cloud Function or the API Gateway spec:
...
Terraform detects the new source zip (content-hashed filename) and redeploys only changed functions. A new API config version is created for spec changes with zero downtime.

---

## Importing the graph into Neo4j

After a document reaches `status: completed`:
